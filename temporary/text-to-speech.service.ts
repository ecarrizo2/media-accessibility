...
export class TextToSpeechService {
  ...
  async processText(text: string): Promise<SpeechEntity> {
    ...
    const existingEntry = await this.speechRepository.getSpeech(text)
    if (existingEntry) {
      this.logger.debug('existing entry data', existingEntry)
      this.logger.info(`Entry found, id: ${existingEntry.id} /processText()`)
    ...
    await this.speechRepository.saveSpeech(speechData)
